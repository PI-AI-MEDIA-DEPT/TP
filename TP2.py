...
class Localization(threading.Thread):
    def __init__(self, url, camid, cs_host, section_name = 'work',model_name="best.pt", ip="127.0.0.1"):
    ...
    def camPreview(self,url):
        ...
        while True:
            try:
                ...
                if rval == False:
                    restart_count +=1
                    print(self.camid, rval ,type(rval) , restart_count)

                    if restart_count >= 600:
                        #time.sleep(1)
                        print("restart", self.camid)
                        self.new_capture()
                        restart_count = 0
    ...
    def new_capture(self):
        self.cap.release()
        time.sleep(2)
        c1 = Connection(host=self.ip, user='jetson', connect_kwargs={'password': 'yahboom'})
  
        try:
            c1.run('docker restart edge')
            time.sleep(2)
...
class Detect():
    def __init__(self):
        self.cap_alive = {}
    def run(self):   
        # self.initialize_fullscreen_window("vis")
        container = []
        rtsp1 = {"rtsp": "rtsp://192.168.33.40:8554/mystream", "cam_id":0, "model":"best.pt", "ip": "192.168.33.40"}
        rtsp2 = {"rtsp": "rtsp://192.168.33.249:8554/mystream", "cam_id":1, "model":"last.pt", "ip": "192.168.33.249"}
        rtsp3 = {"rtsp": "rtsp://192.168.33.8:8554/mystream", "cam_id":2, "model":"best.pt", "ip": "192.168.33.8"}
        rtsp4 = {"rtsp": "rtsp://192.168.33.36:8554/stream1", "cam_id":3, "model":"last.pt", "ip": "192.168.33.36"}
        ...
        recode_out = cv2.VideoWriter('appsrc ! videoconvert' + \
                    ...
                    ' ! rtspclientsink location={}'.format("rtsp://127.0.0.1:8554/TP"),
                    cv2.CAP_GSTREAMER, 0, 15, (1920,1080), True)
        
        container.append(rtsp1)
        container.append(rtsp2)
        container.append(rtsp3)
        container.append(rtsp4)
        net1 = 0
        net2 = 0
        net3 = 0
        net4 = 0
        ...
        cv2.namedWindow("vis", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("vis", 1920, 1080)  # Adjust to your screen resolution
        ...
        while True:        
            ...
            for index_n, n in enumerate(nets):
                ...
                if index_n == 0:
                    ...
                    vis_ori[0:540,0:960] = ori

                if index_n == 1:
                    vis_temp[0:540,960:1920] = vid
                    vis_ori[0:540,960:1920] = ori

                if index_n == 2:
                    vis_temp[540:1080,0:960] = vid
                    vis_ori[540:1080,0:960]  = ori

                if index_n == 3:
                    vis_temp[540:1080,960:1920] = vid
                    vis_ori[540:1080,960:1920] = ori
